...
const CONFIG = {
    ...
    PATHS: {
        EDUCATION_DATA: 'data/education.json'
    }
};
...
async function initializeeducation() {
    try {
        ...
        if (!validateItemData(educationData.items)) {
            throw new Error(createErrorMessage(ErrorTypes.DATA_INVALID, 'education.json'));
        }
        ...
        handleError('Failed to initialize education section', error);
    }
}
...
function setActiveView(view) {
    ...
    if (educationGrid) {
        educationGrid.style.opacity = '0';
        setTimeout(() => {
            renderEducationSection(view);
            educationGrid.style.opacity = '1';
...
async function fetchEducationData() {
    try {
        ...
        const response = await fetch(CONFIG.PATHS.EDUCATION_DATA, {
        ...
        if (!response.ok) {
            if (response.status === 404) {
                throw new Error(createErrorMessage(ErrorTypes.FILE_NOT_FOUND, 'education.json'));
            }
        ...
        if (error.name === 'SyntaxError') {
            throw new Error(createErrorMessage(ErrorTypes.DATA_INVALID, 'invalid JSON format in education.json'));
        }
        throw error;
    }
}
...
// Rendering Functions
function renderEducationSection(viewType) {
    const educationGrid = document.querySelector('.education-grid');
    if (!educationGrid) {
    ...
    educationGrid.dataset.view = viewType;

    const items = window.educationData.items
        .filter(item => item.type === viewType || (viewType === 'professional' && item.type === 'internship'))
        .sort((a, b) => a.rank - b.rank);

    educationGrid.innerHTML = items.map(createItemTile).join('');
...
function createItemTile(item) {
    const { id, schoolDescription, branding, name, role, tags, overview } = item;
    ...
    return `
        <div class="school-tile fade-in" id="${id}" data-school-id="${id}">
            <div class="school-tooltip">${schoolDescription}</div>
...
function initializeModalListeners() {
    const modal = document.getElementById('schoolDetails');
    if (!modal) {
...
function handleTileClick(event) {
    const itemId = event.currentTarget.dataset.schoolId;
...
// Modal Functions
function showSchoolDetails(details) {
    const modal = document.getElementById('schoolDetails');
    if (!modal) {
        console.error('Modal element not found');
        return;
...
function showErrorMessage(message, type = 'error') {
    ...
    educationGrid.insertAdjacentElement('beforebegin', errorDiv);
}
